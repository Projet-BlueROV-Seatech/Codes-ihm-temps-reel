...
def reprojection_error(pts3d, pts2d_undist, K, rvec, tvec):
    """Calcule l'erreur de reprojection en pixels sur des points undistordus."""
    D_zero = np.zeros((5, 1)) # Distorsion nulle car les points sont déjà corrigés
...
def executer_calibration(marq1, marq2, communs):
    ...
    # Caméra 1
    wMc1, c1Mw, rvec1, tvec1 = solvePnP_vers_matrice(wXs_3d, pts1_px, K1, D1)
    
    # Caméra 2
    wMc2, c2Mw, rvec2, tvec2 = solvePnP_vers_matrice(wXs_3d, pts2_px, K2, D2)

    # Erreurs initiales (PnP)
    err1 = reprojection_error(wXs_3d, pts1_ud, K1, rvec1, tvec1)
    err2_pnp = reprojection_error(wXs_3d, pts2_ud, K2, rvec2, tvec2)
    ...
    # Calcul des écarts de l'optimisation
    delta_t = np.linalg.norm(c2Mc1_pnp[:3, 3] - c2Mc1_opt[:3, 3])
    delta_R = np.linalg.norm(c2Mc1_pnp[:3, :3] - c2Mc1_opt[:3, :3], 'fro')

    # Erreur de reprojection Cam2 après optimisation Nelder-Mead
    rvec2_opt, _ = cv2.Rodrigues(c2Mw_opt[:3, :3])
    ...
    return c2Mc1_opt, wMc1, wMc2_opt, err1, err2_opt, delta_t, delta_R, res.fun
...
while True:
    ...
    if stable_frames >= 10:
        ...
        if choix == 'o':
            ...
            erreurs_extrinseques = np.array({
                "reprojection_cam1_px": err1,
                "reprojection_cam2_px": err2,
                "incertitude_translation_m": delta_t,
                "incertitude_rotation_frobenius": delta_R,
                "cout_residuel_optimisation": cout_opt
            }, dtype=object)
